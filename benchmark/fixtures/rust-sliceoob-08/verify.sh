...
#
# Independent verifier for fault_id=rust-sliceoob-08 (see meta.json). The
# fault is fixture-app-rust's catalog_item() (items.rs) no longer
# validating the requested index before indexing CATALOG_ITEMS, so
# GET /items?index=99 panics with "index out of bounds". Checked directly
...
# back as a real status code. Also re-checks a valid index so a "fix" that
# just always returns 400 doesn't also score as correct.
#
