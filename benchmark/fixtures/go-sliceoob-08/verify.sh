...
#
# Independent verifier for fault_id=go-sliceoob-08 (see meta.json). The
# fault is fixture-app-go's handleCatalogItem() (items.go) no longer
# validating the requested index before indexing catalogItems, so
# GET /items?index=99 panics with "index out of range". Checked directly
# against real HTTP behavior, not against any test file Hermes could have
# authored.
#
# Checks the HTTP status code, not "does the process stay alive" -- Go's
# net/http recovers a per-request panic without killing the process, so
# "still alive after the request" would pass even for the *unfixed* code.
# A panicking request comes back to curl as a dropped connection (status
# "000"); only a real completed response comes back as a real status code.
# Also re-checks a valid index so a "fix" that just always returns 400
# doesn't also score as correct.
#
...
BIN="$(mktemp -t revi-verify-go-sliceoob-08-XXXXXX)"
...
valid_body=$(curl --silent --max-time 2 "${ITEMS_URL}?index=1")
if [ "$valid_body" != "gadget" ]; then
  echo "verify: GET /items?index=1 returned '$valid_body', want 'gadget' -- regression in the happy path" >&2
  exit 1
fi
