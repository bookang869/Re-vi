...
#
# Independent verifier for fault_id=node-divzero-02 (see meta.json). The
# fault is fixture-app-node's divideShare() (divide.js) dividing by parts
# without checking for zero, so GET /divide-share?parts=0 lets the
# underlying BigInt division's native "RangeError: Division by zero"
# propagate uncaught (JS's regular Number division doesn't throw on zero,
# unlike Go/Rust/Python -- this fixture uses BigInt specifically so the
# fault_type still matches the other three languages' runtime-exception
# faults). Checked directly against real HTTP behavior, not against any
# test file Hermes could have authored.
#
...
# as a real 500, not a dropped connection. Also re-checks the non-zero
# happy path (parts=4) so a "fix" that just stubs out the whole handler
