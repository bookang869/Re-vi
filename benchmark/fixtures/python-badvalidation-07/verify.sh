...
#
# Independent verifier for fault_id=python-badvalidation-07 (see
# meta.json). The fault is fixture-app-python's format_order() (format.py)
# no longer rejecting a negative amount, so GET /format-order?amount=-5
# returns 200 with a nonsensical order instead of 400. Checked directly
# against real HTTP responses, not against source or any test file Hermes
# could have authored.
#
