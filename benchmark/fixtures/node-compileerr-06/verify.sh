...
#
# Independent verifier for fault_id=node-compileerr-06 (see meta.json).
# The fault is fixture-app-node's stats.js having an unclosed paren, a
# genuine SyntaxError, so `node --check stats.js` fails and the whole app
# (server.js requires stats.js at module load) can't even boot. Checked
# by actually running node --check and hitting the real /average
# endpoint, not by inspecting source or any test file Hermes could have
# authored -- averageOrderValue is wired to HTTP specifically so a "fix"
# that just deletes the broken function (which would also make the file
# parse) doesn't score as correct: it would 404 here instead of
# returning 20.
#
...
echo "verify: node --check succeeded and /average returns 20 -- correctly repaired"
exit 0
