...
#
# Independent verifier for fault_id=python-config-04 (see meta.json). The
# fault is fixture-app-python's max_order_amount() (settings.py) falling
# back to 0 (instead of the documented 100000) when MAX_ORDER_AMOUNT is
# unparsable, so an invalid config value silently rejects every order.
# Checked by actually launching the app with a deliberately invalid
# MAX_ORDER_AMOUNT and hitting the real endpoint, not by inspecting source
# or any test file Hermes could have authored.
#
...
# everything.
MAX_ORDER_AMOUNT=notanumber python3 server.py &
APP_PID=$!
