...
#
# Independent verifier for fault_id=python-divzero-02 (see meta.json). The
# fault is fixture-app-python's divide_share() (divide.py) dividing by
# parts without checking for zero, so GET /divide-share?parts=0 raises
# ZeroDivisionError. Checked directly against real HTTP behavior, not
...
# completed response comes back as a real status code. Also re-checks the
# non-zero happy path (parts=4) so a "fix" that just stubs out the whole
# handler doesn't score as correct.
#
