...
#
# Independent verifier for fault_id=python-compileerr-06 (see meta.json).
# The fault is fixture-app-python's stats.py having an unclosed paren, a
# genuine SyntaxError, so `python3 -m py_compile stats.py` fails and the
# whole app (server.py imports stats at module load) can't even boot.
# Checked by actually running py_compile and hitting the real /average
# endpoint, not by inspecting source or any test file Hermes could have
# authored -- average_order_value is wired to HTTP specifically so a "fix"
# that just deletes the broken function (which would also make the module
# importable) doesn't score as correct: it would 404 here instead of
# returning 20.
#
...
set -uo pipefail   # not -e: a failed compile/check is a verdict (exit 1), not a script bug
