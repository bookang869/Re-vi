...
#
# Independent verifier for fault_id=go-divzero-02 (see meta.json). The
# fault is fixture-app-go's handleDivideShare() (divide.go) dividing by
...
# Also re-checks the non-zero happy path (parts=4) so a "fix" that just
# stubs out the whole handler doesn't score as correct.
#
...
BIN="$(mktemp -t revi-verify-go-divzero-02-XXXXXX)"
