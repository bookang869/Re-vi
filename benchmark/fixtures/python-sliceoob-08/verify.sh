...
#
# Independent verifier for fault_id=python-sliceoob-08 (see meta.json). The
# fault is fixture-app-python's catalog_item() (items.py) no longer
# validating the requested index before indexing CATALOG_ITEMS, so
# GET /items?index=99 raises IndexError. Checked directly against real HTTP
# behavior, not against any test file Hermes could have authored.
#
...
# without killing the process, so "still alive after the request" would
# pass even for the *unfixed* code. A request that raised inside do_GET
# comes back to curl as a dropped connection (status "000"); only a real
# completed response comes back as a real status code. Also re-checks a
# valid index so a "fix" that just always returns 400 doesn't also score
# as correct.
#
