...
#
# Independent verifier for fault_id=go-config-04 (see meta.json). The fault
# is fixture-app-go's handleValidateOrder() (settings.go) ignoring
# strconv.Atoi's error when parsing MAX_ORDER_AMOUNT, so an invalid config
# value silently becomes max=0 instead of falling back to the documented
# 100000 default. Checked by actually launching the binary with a
# deliberately invalid MAX_ORDER_AMOUNT and hitting the real endpoint, not
...
BIN="$(mktemp -t revi-verify-go-config-04-XXXXXX)"
