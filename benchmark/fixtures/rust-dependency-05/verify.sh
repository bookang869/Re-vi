...
#
# Independent verifier for fault_id=rust-dependency-05 (see meta.json). The
# fault is fixture-app-rust's inventory_check() (inventory.rs) no longer
# checking the downstream inventory service's status code before relaying
# its body, so a non-200 dependency response gets forwarded as if it were a
# successful lookup. Checked against a real (mock) downstream HTTP server
# this script starts itself, not against source or any test file Hermes
# could have authored.
#
...
http.server.HTTPServer(("127.0.0.1", 9092), H).serve_forever()
PYEOF
  MOCK_PID=$!
  sleep 0.3
}
...
INVENTORY_URL="http://127.0.0.1:${MOCK_PORT}" "$BIN" &
APP_PID=$!
